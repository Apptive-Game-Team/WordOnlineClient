...
세계수 바람 정령의 상공에서 목표 지점에 개입하는 하강 돌풍.

## 컨셉 설명

하강 돌풍은 세계수 바람 정령의 투하 마법이다. 목표 상공에서 생성되어 낙하한다. 전투에서는 투하 공격 역할을 맡으며, 대상 제한을 이 파일에서 직접 확인하지 못함 대상을 직접 공격 없음 또는 별도 효과 방식으로 다룬다. 추가 특수 이동은 없다. 시각적으로는 넓고 읽기 쉬운 곡선 종이 띠로 흐름과 회전을 표현한다.
...
- Magic: `game/src/main/java/com/wordonline/server/game/domain/magic/implement/drop/WindDropMagic.java`
