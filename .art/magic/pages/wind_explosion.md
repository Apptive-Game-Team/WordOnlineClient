...
세계수 바람 정령의 한순간에 범위를 장악하는 터지는 돌풍.

## 컨셉 설명

터지는 돌풍은 세계수 바람 정령의 범위 폭발 마법이다. 목표 지점을 중심으로 범위 효과를 생성한다. 전투에서는 범위 공격 역할을 맡으며, 대상 제한을 이 파일에서 직접 확인하지 못함 대상을 범위 방식으로 다룬다. 추가 특수 이동은 없다. 시각적으로는 넓고 읽기 쉬운 곡선 종이 띠로 흐름과 회전을 표현한다.
...
| 항목 | 값 |
|---|---|
| 구현 클래스 | `WindExplosionMagic` |
| 상위 클래스 | `AbstractExplosionMagic` |
| 주 프리팹 | `WindExplode` |
| 부가 프리팹 | 없음 또는 소스에서 직접 확인되지 않음 |
| 부착 컴포넌트 | `CircleCollider`, `KnockbackEffectProvider`, `Explode` |
| 파라미터 키 | `RADIUS`, `DAMAGE` |
| 오브젝트 파라미터 | `WIND_EXPLODE` |
...
- Magic: `game/src/main/java/com/wordonline/server/game/domain/magic/implement/explode/WindExplosionMagic.java`
- Prefab initializer: `game/src/main/java/com/wordonline/server/game/domain/object/prefab/implement/explode/WindExplodePrefabInitializer.java`
