...
세계수 풀 정령의 독립 개체로 전장에 합류하는 덩굴 추적자.

## 컨셉 설명

덩굴 추적자은 세계수 풀 정령의 소환 마법이다. 소스에서 지속 이동 AI를 확인하지 못했다. 전투에서는 전투 개체 역할을 맡으며, 지상·공중 대상을 직접 공격 없음 또는 별도 효과 방식으로 다룬다. 추가 특수 이동은 없다. 시각적으로는 잎·씨앗·덩굴의 겹친 컷페이퍼 형태로 생장 방향을 보여준다.
...
| 항목 | 값 |
|---|---|
| 구현 클래스 | `VineSpiritMagic` |
| 상위 클래스 | `AbstractSingleSpawnMagic` |
| 주 프리팹 | `VineSpirit` |
| 부가 프리팹 | 없음 또는 소스에서 직접 확인되지 않음 |
| 부착 컴포넌트 | `RigidBody`, `ZPhysics`, `CircleCollider`, `EffectProvideProjectileRangeAttackMob`, `CommonEffectReceiver` |
| 파라미터 키 | `MASS`, `RADIUS`, `HP`, `SPEED`, `DAMAGE`, `ATTACK_INTERVAL`, `ATTACK_RANGE` |
| 오브젝트 파라미터 | `VINE_SPIRIT` |
...
- Magic: `game/src/main/java/com/wordonline/server/game/domain/magic/implement/spawn/VineSpiritMagic.java`
- Prefab initializer: `game/src/main/java/com/wordonline/server/game/domain/object/prefab/implement/misc/third/VineSpiritPrefabInitializer.java`
