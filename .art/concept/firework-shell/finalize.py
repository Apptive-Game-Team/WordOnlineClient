#!/usr/bin/env python3
"""Finalize a firework_shell candidate that already carries real alpha.

Unlike some earlier finalize scripts in this repo, this one never mattes a
painted background away. It only works on a source that already has a true
alpha channel from the generator (see the "-alpha" suffix on the accepted
attempt file). If the source is opaque or carries a painted checkerboard,
this script refuses and says so — regenerate instead.
"""
...
def main() -> None:
    ...
    parser.add_argument("--max-dim", type=int, default=128, help="max canvas dimension (tier cap)")
    ...
    scale = min(args.max_dim / cropped.width, args.max_dim / cropped.height)
